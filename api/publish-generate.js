...
const { cors, parseJsonBody, getBearerToken } = require('../server-lib/payments-util.js');
...
module.exports = async (req, res) => {
  ...
  if (!token) {
    return res.status(401).json({ error: 'auth_required', message: 'Sign in before using hosted AI generation.' });
  }
  if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
    return res.status(503).json({ error: 'config', message: 'Supabase service credentials missing for hosted generation.' });
  }
