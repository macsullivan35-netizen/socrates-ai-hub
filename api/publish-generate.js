...
module.exports = async (req, res) => {
  ...
  if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
    return res.status(503).json({
      error: 'auth_config',
      message: 'Hosted generation requires Supabase auth configuration on the API server.',
    });
  }
  ...
  if (!token) {
    return res.status(401).json({
      error: 'auth',
      message: 'Sign in before using hosted generation, or paste your own OpenAI key.',
    });
  }
  ...
  if (userErr || !user) {
    return res.status(401).json({
      error: 'auth',
      message: 'Invalid session. Sign in again, or paste your own OpenAI key.',
    });
  }
