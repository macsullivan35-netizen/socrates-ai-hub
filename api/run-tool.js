  ...
  if (Number.isInteger(asNum) && asNum >= 1 && asNum <= 12) {
    ...
    if (priceNum > 0) {
      ...
      const entitlement = body.entitlement || {
        sessionId: body.sessionId || body.checkoutSessionId,
        nonce: body.unlockNonce,
      };
      const access = await verifyPaidToolEntitlement(stripeFactory(process.env.STRIPE_SECRET_KEY), entitlement, toolIdRaw);
      if (!access.ok) {
