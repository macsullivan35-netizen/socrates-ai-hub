  ...
  try {
    ...
    if (String(session.metadata?.tool_id || '') !== String(toolId)) {
      return {
        ok: false,
        status: 403,
        body: { error: 'payment_mismatch', message: 'Checkout session does not unlock this tool.' },
      };
    }
    return { ok: true };
  } catch (err) {
    return {
      ok: false,
      status: 402,
      body: { error: 'payment_invalid', message: err.message || 'Checkout session could not be verified.' },
    };
  }
}
...
module.exports = async (req, res) => {
  ...
  if (Number.isInteger(asNum) && asNum >= 1 && asNum <= 12) {
    ...
    if (Number(tool.price) > 0) {
      const paidAccess = await verifyPaidToolAccess(toolIdRaw, checkoutSessionId);
      if (!paidAccess.ok) {
