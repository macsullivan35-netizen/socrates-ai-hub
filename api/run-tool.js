...
//      SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY (for UUID tools), STRIPE_SECRET_KEY (paid UUID tools),
...
async function verifyPaidCheckoutSession(sessionId, toolId) {
  ...
  if (!process.env.STRIPE_SECRET_KEY) {
    return { ok: false, status: 503, error: 'config', message: 'Stripe is not configured for paid tool access checks.' };
  }
  ...
  try {
    ...
    if (session.payment_status !== 'paid' || String(session.metadata?.tool_id || '') !== String(toolId)) {
      return { ok: false, status: 402, error: 'payment_required', message: 'Checkout session does not unlock this tool.' };
    }
    return { ok: true };
  } catch (e) {
...
module.exports = async (req, res) => {
  ...
  if (Number.isInteger(asNum) && asNum >= 1 && asNum <= 12) {
    ...
    if ((Number(tool.price) || 0) > 0) {
      const access = await verifyPaidCheckoutSession(checkoutSessionId, toolIdRaw);
      if (!access.ok) {
