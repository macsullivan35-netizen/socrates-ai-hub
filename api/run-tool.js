// POST { toolId, userMessage, model?: "gpt" | "claude", sessionId?: "cs_..." } — run a published tool via platform API keys (no visitor key).
// Env: OPENAI_API_KEY (required for model gpt), ANTHROPIC_API_KEY (required for model claude),
//      SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY (for UUID tools), STRIPE_SECRET_KEY (required for paid UUID tools),
...
module.exports = async (req, res) => {
  ...
  const checkoutSessionId = body.sessionId != null ? String(body.sessionId).trim() : '';
  ...
  if (Number.isInteger(asNum) && asNum >= 1 && asNum <= 12) {
    ...
    if (Number.isFinite(priceNum) && priceNum > 0) {
      const paidAccess = await verifyPaidCheckoutSession(process.env.STRIPE_SECRET_KEY, toolIdRaw, checkoutSessionId);
      if (!paidAccess.ok) {
