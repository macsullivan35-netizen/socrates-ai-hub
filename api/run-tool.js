// POST { toolId, userMessage, model?: "gpt" | "claude", accessSessionId?: "cs_..." } — run a published tool via platform API keys (no visitor key).
...
async function verifyPaidToolAccess(toolId, sessionId) {
  const checkoutSessionId = sessionId != null ? String(sessionId).trim() : '';
  ...
  if (!process.env.STRIPE_SECRET_KEY) {
    return {
      ...
      message: 'Paid tool access validation is not configured.',
    };
  }
...
module.exports = async (req, res) => {
  ...
  if (Number.isInteger(asNum) && asNum >= 1 && asNum <= 12) {
    ...
    if (priceNum > 0) {
      try {
        const access = await verifyPaidToolAccess(toolIdRaw, body.accessSessionId);
        if (!access.ok) {
        ...
        return res.status(402).json({
          error: 'payment_required',
          message: e.message || 'Could not verify paid tool access.',
        });
      }
    }
