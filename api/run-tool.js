...
async function verifyCheckoutSessionForTool(sessionId, toolId) {
  if (!sessionId) {
    return {
      ok: false,
      status: 402,
      body: { error: 'payment_required', message: 'Complete checkout to run this paid tool.' },
    };
  }
  if (!process.env.STRIPE_SECRET_KEY) {
    return {
      ok: false,
      status: 503,
      body: { error: 'config', message: 'Stripe is not configured for paid tool runs.' },
    };
  }
  ...
  try {
    ...
    if (session.payment_status !== 'paid') {
      return {
        ok: false,
        status: 402,
        body: { error: 'payment_required', message: 'Checkout session is not paid.' },
      };
    }
    if (String(session.metadata?.tool_id || '') !== String(toolId)) {
      return {
        ok: false,
        status: 403,
        body: { error: 'invalid_purchase', message: 'Checkout session does not unlock this tool.' },
      };
    }
    return { ok: true };
  } catch (err) {
    return {
      ok: false,
      status: 403,
      body: { error: 'invalid_purchase', message: err.message || 'Could not verify checkout session.' },
    };
  }
}
...
module.exports = async (req, res) => {
  ...
  if (Number.isInteger(asNum) && asNum >= 1 && asNum <= 12) {
    ...
    if (priceNum > 0) {
      const paid = await verifyCheckoutSessionForTool(checkoutSessionId, toolIdRaw);
