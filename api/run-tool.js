...
async function verifyPaidToolSession(toolId, sessionId) {
  ...
  if (Number.isInteger(asNum) && asNum >= 1 && asNum <= 12) {
    ...
    if (priceNum > 0) {
      try {
        ...
        const paid = await verifyPaidToolSession(toolIdRaw, checkoutSessionId);
        if (!paid) {
          return res.status(402).json({
            error: 'payment_required',
            message: 'Complete checkout for this paid tool before running it.',
          });
        }
      } catch {
        return res.status(402).json({
